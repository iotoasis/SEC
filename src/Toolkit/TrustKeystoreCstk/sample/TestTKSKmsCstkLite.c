...
#ifdef WIN32
#pragma comment(lib, "TKSKmsCstkLite.lib")
#endif

int TKSKmsCstkLite_init(void **ppCtx, char* szPath);
int TKSKmsCstkLite_hash(void *pCtx, int nAlgo, unsigned char* pMsg, int nMsgLen, unsigned char* pHash, int nHashLen);
int TKSKmsCstkLite_block(void *pCtx);
int TKSKmsCstkLite_hmac(void *pCtx);
int TKSKmsCstkLite_ReqKey(void *pCtx, unsigned char  *pOut, int *nOutLen);
int TKSKmsCstkLite_ResKey(void *pCtx, unsigned char  *pIn, int nInLen, unsigned char  *pOut, int *nOutLen);
int TKSKmsCstkLite_SetKey(void *pCtx, unsigned char  *pIn, int nInLen);
int TKSKmsCstkLite_GetKey(void *pCtx);
...
void main()
{
	...
	ret = TKSKmsCstkLite_hash(pCtx, nHashAlgo, TobeHashData, sizeof(TobeHashData), HashedData, sizeof(HashedData));
	if(ret)
	{
		goto err;
	}
...
before_init:
	ret = TKSKmsCstkLite_init(&pCtx, szConfPath);
	if(ret)
	{
		goto err;
	}

	//ret = TKSKmsCstkLite_block(pCtx);
	if(ret)
	{
		if(ret == -4000)
			goto before_init;
	...
	*/
	...
	// G/W 에서는 지원하지 않는 API
	/*
	ret = TKSKmsCstkLite_ReqKey(pCtx, ReqKeyMsg, &nReqKeyMsgLen);
	if(ret)
	{
		goto err;
	}
	*/
	...
	//ret = TKSKmsCstkLite_ResKey(pCtx, ReqKeyMsg, nReqKeyMsgLen, ResKeyMsg, &nResKeyMsgLen);
	if(ret)
	{
		goto err;
	}
	...
	// G/W 에서는 지원하지 않는 API
	/*
	ret = TKSKmsCstkLite_SetKey(pCtx, ResKeyMsg, nResKeyMsgLen);
	if(ret)
	{
		goto err;
	}
	*/
	ret = TKSKmsCstkLite_GetKey(pCtx);
	if(ret)
	{
		goto err;
	}
