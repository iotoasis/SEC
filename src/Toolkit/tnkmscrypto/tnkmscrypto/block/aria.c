...
/* 사용 플랫폼의 endian 특성에 따라 L_ENDIAN 혹은
 * B_ENDIAN 둘 중 하나를 정의해야 컴파일 됩니다.
...
/* BY(X, Y)는 U32 X의 Y번째 바이트
 * BRF(T,R)은 T>>R의 하위 1바이트
 * WO(X, Y)는 U8 array X를 U32 array로 간주할 때 Y번째 U32
 */

#define BY(X,Y) (((U8 *)(&X))[Y])
#define BRF(T,R) ((U8)((T)>>(R)))
#define WO(X,Y) (((U32 *)(X))[Y])
...
/* U8 array를 Word에 싣는 함수.  LITTLE_ENDIAN의 경우
 * 엔디안 변환 과정을 거친다. */
#if defined(WIN32) || defined(L_ENDIAN)
...
#define P(T0,T1,T2,T3) {					\
  ...
  * 결과적으로 U32 단위로 endian을 뒤집은 결과를 준다.
  * 즉, FO, FE는 B_ENDIAN 환경에서는 ARIA spec과 동일한 결과를,
  * L_ENDIAN 환경에서는 ARIA spec에서 정의한 변환+endian 변환을
  * 준다. */
...
/*	
	Name : S_ARIA_Encrypt
	Description: ARIA 블럭(16byte) 암호화
	Parameters
	[in] key : ARIA 키 구조체
	[in] i : 암호화 할 데이터
	[out] o : 암호화 된 데이터
	...
	Note : 
*/
void S_ARIA_Encrypt(ARIA_KEY *key, const U8 *i, U8 *o) 
{
  register U32 t0, t1, t2, t3;
  const U8 *rk = key->rd_key;
  int Nr = (key->keyBits + 256 ) / 32;
  ...
  /* 최종 라운드는 특별함 */
#if defined(WIN32) || defined(L_ENDIAN)
  o[ 0] = (U8)(X1[BRF(t0,24)]   ) ^ rk[ 3];
  o[ 1] = (U8)(X2[BRF(t0,16)]>>8) ^ rk[ 2];
  o[ 2] = (U8)(S1[BRF(t0, 8)]   ) ^ rk[ 1];
  o[ 3] = (U8)(S2[BRF(t0, 0)]   ) ^ rk[ 0];
  o[ 4] = (U8)(X1[BRF(t1,24)]   ) ^ rk[ 7];
  o[ 5] = (U8)(X2[BRF(t1,16)]>>8) ^ rk[ 6];
  o[ 6] = (U8)(S1[BRF(t1, 8)]   ) ^ rk[ 5];
  o[ 7] = (U8)(S2[BRF(t1, 0)]   ) ^ rk[ 4];
  o[ 8] = (U8)(X1[BRF(t2,24)]   ) ^ rk[11];
  o[ 9] = (U8)(X2[BRF(t2,16)]>>8) ^ rk[10];
  o[10] = (U8)(S1[BRF(t2, 8)]   ) ^ rk[ 9];
  o[11] = (U8)(S2[BRF(t2, 0)]   ) ^ rk[ 8];
  o[12] = (U8)(X1[BRF(t3,24)]   ) ^ rk[15];
  o[13] = (U8)(X2[BRF(t3,16)]>>8) ^ rk[14];
  o[14] = (U8)(S1[BRF(t3, 8)]   ) ^ rk[13];
  o[15] = (U8)(S2[BRF(t3, 0)]   ) ^ rk[12];
#else
  o[ 0] = (U8)(X1[BRF(t0,24)]   );	
  o[ 1] = (U8)(X2[BRF(t0,16)]>>8);	
  o[ 2] = (U8)(S1[BRF(t0, 8)]   );	
  o[ 3] = (U8)(S2[BRF(t0, 0)]   );	
  o[ 4] = (U8)(X1[BRF(t1,24)]   );	
  o[ 5] = (U8)(X2[BRF(t1,16)]>>8);	
  o[ 6] = (U8)(S1[BRF(t1, 8)]   );	
  o[ 7] = (U8)(S2[BRF(t1, 0)]   );	
  o[ 8] = (U8)(X1[BRF(t2,24)]   );	
  o[ 9] = (U8)(X2[BRF(t2,16)]>>8);	
  o[10] = (U8)(S1[BRF(t2, 8)]   );	
  o[11] = (U8)(S2[BRF(t2, 0)]   );	
  o[12] = (U8)(X1[BRF(t3,24)]   );	
  o[13] = (U8)(X2[BRF(t3,16)]>>8);	
  o[14] = (U8)(S1[BRF(t3, 8)]   );	
  o[15] = (U8)(S2[BRF(t3, 0)]   );	
...
/*	
	Name : S_ARIA_Decrypt
	Description: ARIA 블럭(16byte) 복호화
	Parameters
	[in] key : ARIA 키 구조체
	[in] i : 복호화 할 데이터
	[out] o : 복호화 된 데이터
	...
	Note : 
*/
void S_ARIA_Decrypt(ARIA_KEY *key, const U8 *i, U8 *o)
{
  S_ARIA_Encrypt(key, i, o);
}

/*	
	Name : S_ARIA_Encrypt_KeySchedule
	Description: 암호키를 암호화를 위해 ARIA 키구조체에 설정한다
	Parameters
	[out] key : ARIA 키 구조체
	[in] mk : 암호키
	[in] keyBits : 암호키 길이
	Return Value : 설정된 키의 32bit 블럭 갯수를 반환한다
	Note : 
*/
...
/*	
	Name : S_ARIA_Decrypt_KeySchedule
	Description: 암호키를 복호화를 위해 ARIA 키구조체에 설정한다
	Parameters
	[out] key : ARIA 키 구조체
	[in] mk : 암호키
	[in] keyBits : 암호키 길이
...
int S_ARIA_Decrypt_KeySchedule(ARIA_KEY *key, const U8 *mk, int keyBits) 
{
  ...
  rValue=S_ARIA_Encrypt_KeySchedule(key, mk, keyBits);
  a=(U32 *)(key->rd_key);  z=a+rValue*4;
