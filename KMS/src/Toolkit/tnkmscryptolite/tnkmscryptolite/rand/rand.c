...
/*
	Name : S_GenerateRandom
 	Description : 선택 모드에 따라서 LSH256 해시 알고리즘을 사용하여 랜덤 값을 생성 한다.
 	Parameters
 	[out] out : 생성된 랜덤 데이터
	[in] bytes : 생성할 랜덤 데이터의 길이
	[in] mode : 선택 모드(LSH256 : 0)
 	Return Value :
 	Note :
 */
void S_GenerateRandom(U8 *out, U32 bytes, int mode)
{
	...
	{
		S_GenerateRandom_LSH256(out, bytes); 
	}
}
...
void S_RAND_Seed(U8 *buf, int num)
{
	...
	for (i = 0; i < num; i += MD_DIGEST_LENGTH)     
	{
		...
		lsh256_init(&m);
		lsh256_update(&m, md, MD_DIGEST_LENGTH);
		k = st_idx + j - STATE_SIZE;
		if (k > 0)
		{
			lsh256_update(&m, &(state[st_idx]), j - k);
			lsh256_update(&m, &(state[0]), k);
		}
		else
			lsh256_update(&m, &(state[st_idx]), j);
			
		lsh256_update(&m, buf, j);
		lsh256_final(&m, md);
...
void S_RAND_Bytes(U8 *buf, int num)
{
	...
	while (num > 0)
	{
		...
		num -= j;
		lsh256_init(&m);
		lsh256_update(&m, local_md, MD_DIGEST_LENGTH);
		lsh256_update(&m, (unsigned char *)&(md_c[0]), sizeof(md_c));

		k = (st_idx + MD_DIGEST_LENGTH / 2) - st_num;
		if (k > 0)
		{
			lsh256_update(&m, &(state[st_idx]), MD_DIGEST_LENGTH / 2 - k);
			lsh256_update(&m, &(state[0]), k);
		}
		else
			lsh256_update(&m, &(state[st_idx]), MD_DIGEST_LENGTH / 2);
		lsh256_final(&m, local_md);
