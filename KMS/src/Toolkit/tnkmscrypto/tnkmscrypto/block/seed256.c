...
#define GetB0(A)  ( (U8)((A)    ) )
#define GetB1(A)  ( (U8)((A)>> 8) )
#define GetB2(A)  ( (U8)((A)>>16) )
#define GetB3(A)  ( (U8)((A)>>24) )

#define SeedRound(L0, L1, R0, R1, K) {             \
    T0 = R0 ^ (K)[0];                              \
    T1 = R1 ^ (K)[1];                              \
    T1 ^= T0;                                      \
    T1 = S_SS0[GetB0(T1)] ^ S_SS1[GetB1(T1)] ^         \
         S_SS2[GetB2(T1)] ^ S_SS3[GetB3(T1)];          \
    T0 += T1;                                      \
    T0 = S_SS0[GetB0(T0)] ^ S_SS1[GetB1(T0)] ^         \
         S_SS2[GetB2(T0)] ^ S_SS3[GetB3(T0)];          \
    T1 += T0;                                      \
    T1 = S_SS0[GetB0(T1)] ^ S_SS1[GetB1(T1)] ^         \
         S_SS2[GetB2(T1)] ^ S_SS3[GetB3(T1)];          \
    T0 += T1;                                      \
...
#define EndianChange(dwS)                       \
    ( (ROTL((dwS),  8) & (U32)0x00ff00ff) |   \
      (ROTL((dwS), 24) & (U32)0xff00ff00) )
...
/************************ Block Encryption *************************/
/*
	Name : S_SEED256_Encrypt
 	Description : 256비트 키를이용한 SEED 알고리즘으로 입력 데이터를 암호화 한다.
 	Parameters
 	[in/out] pbData : 암호화 할 데이터 / 암호화 된 데이터	
	[in] key : 암호화 키 값
 	Return Value :
 	Note :
 */
void S_SEED256_Encrypt(U8 *pbData, SEED256_KEY *key)
{
	...
	U32 L0, L1, R0, R1, T0, T1, *K = key->data;
...
/*
	Name : S_SEED256_Decrypt
 	Description : 256비트 키를이용한 SEED 알고리즘으로 입력 데이터를 복호화 한다.
 	Parameters
 	[in/out] pbData : 복호화 할 데이터 / 복호화 된 데이터	
	[in] key : 복호화 키 값
 	Return Value :
 	Note :
 */
...
#define EncRoundKeyUpdate0(K, A, B, C, D, E, F, G, H, KC, rot) {  \
    ...
    T0 = (((A + C) ^ E ) - F) ^ KC;              \
    T1 = (((B - D) ^ G ) + H) ^ KC;              \
    (K)[0] = S_SS0[GetB0(T0)] ^ S_SS1[GetB1(T0)] ^   \
             S_SS2[GetB2(T0)] ^ S_SS3[GetB3(T0)];    \
    (K)[1] = S_SS0[GetB0(T1)] ^ S_SS1[GetB1(T1)] ^   \
             S_SS2[GetB2(T1)] ^ S_SS3[GetB3(T1)];    \
}
...
/*
	Name : S_SEED256_KeySchedule
 	Description : 256비트 키를이용한 SEED 라운드키를 생성 한다.
 	Parameters
 	[in/out] key : 암/복호화 라운드 키 값	
	[in] pbUserKey : 암/복호화 키 값
 	Return Value :
 	Note :
*/
void S_SEED256_KeySchedule(SEED256_KEY *key, U8 *pbUserKey)
{
  U32 A, B, C, D, E, F, G, H, T0, T1, *K=key->data;
  A = ((U32 *)pbUserKey)[0];
  B = ((U32 *)pbUserKey)[1];
  C = ((U32 *)pbUserKey)[2];
  D = ((U32 *)pbUserKey)[3];
