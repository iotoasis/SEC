...
int CM_RegDeviceGetDN(Client_CTX *pCtx, char *szDeviceID, char **szDN, char **szKeyAlgo, char **szKeyLength)
{
	...
	sprintf(szMsg, "DEVICEID=%s", szDeviceID);
...
int CM_IssueCertSimple(Client_CTX *pCtx, char *szDeviceID, char *szDN, char *szKeyAlgo, char *szKeyLength, char *szPass, char **szCert, char **szPriKey)
{
	...
	sprintf(szMsg, "USERTYPE=%s&ID=%s&CSR=%s", TYPE_DEVICE, szDeviceID, pszCSR);
...
#ifdef _DEMO
	{
		printf("\n4. Receive and Save Certificate.\n");
		printf("Cert : \n%s\n", *szCert);
	}
#endif
...
#ifdef _DEMO
	{
		printf("\n6. Send digital signature to CA Server.\n");
	}
#endif
...
#ifdef _DEMO
	{
		printf("\n7. Receive authkey from CA Server.\n");
		printf("Authkey : %s\n", *szAuthKey);
	}
#endif
