...
#ifdef WIN32
#pragma comment(lib, "TKSKmsCstk.lib")
#endif

int TKSKmsCstk_init(void **ppCtx, char* szPath);
int TKSKmsCstk_hash(void *pCtx, int nAlgo, unsigned char* pMsg, int nMsgLen, unsigned char* pHash, int nHashLen);
int TKSKmsCstk_block(void *pCtx);
int TKSKmsCstk_hmac(void *pCtx);
int TKSKmsCstk_ReqKey(void *pCtx, unsigned char  *pOut, int *nOutLen);
int TKSKmsCstk_ResKey(void *pCtx, unsigned char  *pIn, int nInLen, unsigned char  *pOut, int *nOutLen);
int TKSKmsCstk_SetKey(void *pCtx, unsigned char  *pIn, int nInLen);
int TKSKmsCstk_GetKey(void *pCtx);
void TKSKmsCstk_final(void **ppCtx);
...
void main()
{
	...
	ret = TKSKmsCstk_hash(pCtx, nHashAlgo, TobeHashData, sizeof(TobeHashData), HashedData, sizeof(HashedData));
	if(ret)
	{
		goto err;
	}
...
before_init:
	ret = TKSKmsCstk_init(&pCtx, szConfPath);
	if(ret)
	{
		goto err;
	}

	ret = TKSKmsCstk_block(pCtx);
	if(ret)
	{
		if(ret == -4000)
			goto before_init;
	...
	// G/W 에서는 지원하지 않는 API
	/*
	ret = TKSKmsCstk_ReqKey(pCtx, ReqKeyMsg, &nReqKeyMsgLen);
	if(ret)
	{
		goto err;
	}
	*/
	...
	//ret = TKSKmsCstk_ResKey(pCtx, ReqKeyMsg, nReqKeyMsgLen, ResKeyMsg, &nResKeyMsgLen);
	if(ret)
	{
		goto err;
	}
	...
	// G/W 에서는 지원하지 않는 API
	/*
	ret = TKSKmsCstk_SetKey(pCtx, ResKeyMsg, nResKeyMsgLen);
	if(ret)
	{
		goto err;
	}
