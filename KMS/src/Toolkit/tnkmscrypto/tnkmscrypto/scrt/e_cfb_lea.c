...
/* LEA 192bit CFB128 모드 알고리즘의 특성에 알맞는 값들을 structure 구조에 저장 */
static SCRT_CIPHER d_cfb128_192_lea=
{
	...
	24, /* key length */
	16,  /* iv length */
	S_lea_cfb128_192_init_key, /* iv, key setting 함수 */
	S_lea_cfb128_cipher,   /* 암복호화 함수 */
	NULL,				   /* 마무리 함수 */
	sizeof(SCRT_CIPHER_CTX)-sizeof((((SCRT_CIPHER_CTX *)NULL)->c))+ /* structure size */
		sizeof((((SCRT_CIPHER_CTX *)NULL)->c.lea_ks))
...
/* LEA 256bit CFB128 모드 알고리즘의 특성에 알맞는 값들을 structure 구조에 저장 */
static SCRT_CIPHER d_cfb128_256_lea=
{
	...
	32, /* key length */
	16,  /* iv length */
	S_lea_cfb128_256_init_key, /* iv, key setting 함수 */
	S_lea_cfb128_cipher,   /* 암복호화 함수 */
	NULL,				   /* 마무리 함수 */
	sizeof(SCRT_CIPHER_CTX)-sizeof((((SCRT_CIPHER_CTX *)NULL)->c))+ /* structure size */
		sizeof((((SCRT_CIPHER_CTX *)NULL)->c.lea_ks))
...
/*	
	...
	Description: LEA 192 bit CFB128 모드 암호알고리즘을 사용하기 위한 정보를 담은 구조체를 반환한다
	Parameters
	Return Value : d_cfb128_192_lea 구조체를 반환한다
	Note :
*/
...
/*	
	...
	Description: LEA 256 bit CFB128 모드 암호알고리즘을 사용하기 위한 정보를 담은 구조체를 반환한다
	Parameters
	Return Value : d_cfb128_256_lea 구조체를 반환한다
	Note :
*/
...
/*	
	...
	Description: 암호키 및 초기벡터를 이용하여 LEA 192bit CFB128 모드 암호알고리즘에 필요한 키 구조체 및 알고리즘 정보를 설정한다
	Parameters
	[out] ctx : LEA 암/복호화를 위한 LEA_KEY 키 구조체 및 정보를 담은 구조체
	[in] key : 암호키 데이터
	[in] iv : 초기벡터
...
/*	
	...
	Description: 암호키 및 초기벡터를 이용하여 LEA 256bit CFB128 모드 암호알고리즘에 필요한 키 구조체 및 알고리즘 정보를 설정한다
	Parameters
	[out] ctx : LEA 암/복호화를 위한 LEA_KEY 키 구조체 및 정보를 담은 구조체
	[in] key : 암호키 데이터
	[in] iv : 초기벡터
